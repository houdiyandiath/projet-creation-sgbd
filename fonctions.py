...
def estPrimaryKey(Monfichier,nom_colonne,nom_table):
 f=open("root_test1.json",'r')
 dic=json.load(f)
 for j in range(len(dic[nom_table][0][nom_colonne])):
  if dic[nom_table][0][nom_colonne][j].upper()=="PRIMARY KEY":
   return 1
 return 0

#print estPrimaryKey("root_test1.json","v","test1_4")
...
def createTable(nom_table,champs,base_courante,nom_user):
        liste_colonnes=[]
        liste_types=[]
        liste_contraintes=[]
...
def insertIntoTable(nombase,nomuser,nametable,values):
     tab=values.split(',')
     Monfichier=nomuser+'_'+nombase+'.json'
     if (tableexiste(Monfichier,nametable)):
        dic=dict()
        tableau=[]
        g=open(Monfichier,'r')
        dic=json.load(g)
        dico=dict()
        colonne_correspondante=-1
        for j in dic[nametable][0].keys():
            colonne_correspondante+=1
            #print 'Pour {0}:'.format(j)
            if(estPrimaryKey(dic,j,nametable)==1):
                                k=1
                                while(k<len(dic[nametable])):
                                 p=k+1
                                 while (p<len(dic[nametable])):
                                  if tab[colonne_correspondante]==dic[nametable][p][j]:
                                   return "This row already exists."
                                  p+=1
                                 k+=1
                                tableau.append(tab[colonne_correspondante])
                                dico[j]=tab[colonne_correspondante]
            else:
                                tableau.append(tab[colonne_correspondante])
                                dico[j]=tab[colonne_correspondante]

        dic[nametable].append(dico)
        k=open(Monfichier,'w')
        json.dump(dic, k, indent=4)
        return "Row affected."
     else:
        return 'Inexisting table.'
#values="4,1"
#print insertIntoTable('test1','root','test1_4',values)
